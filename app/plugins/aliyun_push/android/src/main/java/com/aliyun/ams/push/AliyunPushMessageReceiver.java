...
public class AliyunPushMessageReceiver extends MessageReceiver {
	...
	public NotificationConfigure hookNotificationBuild() {
		return new NotificationConfigure() {
			...
			public void configBuilder(Notification.Builder builder, PushData pushData) {
				// 小米等厂商会按通知自身的 priority/category 判定是否放入
				// “不重要通知”并决定是否弹横幅：显式提到最高级，并标记为
				// 消息类（聊天/信息类通知在厂商过滤器中通常直接放行横幅）。
	...
	public void onNotification(Context context, String title, String summary,
		...
		callFlutterIfAttached("onNotification", arguments);
	}
	...
	protected void onNotificationReceivedInApp(Context context, String title, String summary,
		...
		callFlutterIfAttached("onNotificationReceivedInApp", arguments);
	}
	...
	public void onMessage(Context context, CPushMessage cPushMessage) {
		...
		callFlutterIfAttached("onMessage", arguments);
	}
	...
	public void onNotificationOpened(Context context, String title, String summary,
		...
		arguments.put("summary", summary);
		arguments.put("extraMap", extraMap);
		callFlutterIfAttached("onNotificationOpened", arguments);
	}
	...
	public void onNotificationRemoved(Context context, String messageId) {
		...
		callFlutterIfAttached("onNotificationRemoved", arguments);
	}
	...
	protected void onNotificationClickedWithNoAction(Context context, String title, String summary
		...
		arguments.put("summary", summary);
		arguments.put("extraMap", extraMap);
		callFlutterIfAttached("onNotificationClickedWithNoAction", arguments);
	}

	private void callFlutterIfAttached(String method, Map<String, Object> arguments) {
		// The Android receiver also runs while Flutter is detached (for example
		// after the app was killed). Notification display must not depend on a
		// live Flutter engine, so callbacks are best-effort only.
