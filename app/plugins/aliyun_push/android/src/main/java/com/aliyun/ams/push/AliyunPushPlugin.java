...
public class AliyunPushPlugin implements FlutterPlugin, MethodCallHandler {
	...
	public void callFlutterMethod(String method, Map<String, Object> arguments) {
		final MethodChannel attachedChannel = channel;
		if (TextUtils.isEmpty(method) || attachedChannel == null) {
			return;
		}
		...
		handler.post(() -> {
			...
			try {
				attachedChannel.invokeMethod(method, arguments);
			} catch (RuntimeException error) {
				AliyunPushLog.e(TAG, Log.getStackTraceString(error));
			}
		});
	}
	...
	public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
		...
		if ("initPush".equals(methodName)) {
			...
			if (enabled == null) {
				handleError(result, CODE_PARAM_ILLEGAL, "enabled is required");
				return;
			}
	...
	private void initThirdPush(Result result) {
		handleError(result, CODE_NOT_SUPPORT,
				"Third-party vendor channels are not configured in this build");
	}
